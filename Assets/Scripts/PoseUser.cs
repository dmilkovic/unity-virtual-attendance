...
public class PoseUser : MonoBehaviour
{
    ...
    private void UpdateBone(Astra.Body body)
    {
        for (int i = 0; i < body.Joints.Length; ++i)
        {
            ...
            Joints[i].transform.rotation = _currentRotations[i] = Quaternion.Slerp(_currentRotations[i], _boneRotation[i] * _initialRotations[i], Time.deltaTime * RotationDamping);
        }
    }
